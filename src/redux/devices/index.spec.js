...
describe('devices actions', () => {
  it('toggle should create TOGGLE action', () => {
    expect(devices.toggle(device1.mac, true, true, 'Test')).toEqual({
      type: devices.TOGGLE,
      mac: device1.mac,
      active: true,
      loading: true,
  ...
  it('editName should create EDIT_NAME action', () => {
    expect(devices.editName(device1.mac, 'changed')).toEqual({
      type: devices.EDIT_NAME,
      mac: device1.mac,
      name: 'changed',
    })
  })
  ...
  it('remove should create REMOVE action', () => {
    expect(devices.remove(device1.mac)).toEqual({
      type: devices.REMOVE,
      mac: device1.mac,
    })
  })
...
describe('devices reducer', () => {
  ...
  it('should handle TOGGLE', () => {
    expect(
      devices.default(
        ...
        devices.toggle(device1.mac, true, true, 'Test'),
      ),
    ).toEqual({
      [device1.mac]: {
        ...device1,
  ...
  it('should handle EDIT_NAME', () => {
    expect(
      devices.default(
        ...
        devices.editName(device1.mac, 'changed'),
      ),
    ).toEqual({
      [device1.mac]: {
        ...device1,
  ...
  it('should handle REFRESH', () => {
    expect(
      devices.default({}, devices.refresh([device1, device2], true, 'Test')),
    ).toEqual({
      ...devices.initialState,
      [device1.mac]: device1,
      [device2.mac]: device2,
